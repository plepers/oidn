...
bool HDRLoader::write( const char *fileName, HDRLoaderResult* res )
{
   

   int numRows = res->width;
   int numCols = res->height;
   ...
   RGBE_WritePixels(fpout, res->cols, numPixels);
   int err = fclose(fpout);
   if( err ){
      return false;
   }
